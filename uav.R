...
# devtools::install_github("caiohamamura/flightplanning-R", dependencies=TRUE))

Wysokosc <- 100
ROI_shape <- "uav_planner_testing_data.zip"
...
litchi.plan = function(roi, output,
  ...
  if (flight.lines.angle != -1) {
    ...
    minBbox = flightplanning:::getMinBBox(vertices)
  }
  width = minBbox$width
